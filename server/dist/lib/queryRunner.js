...
export async function emitEventToBus(sessionId, evt) {
    ...
    if (evt.type === "done") {
        await accumulateTokens(sessionId, evt.inputTokens, evt.outputTokens, evt.durationMs).catch((err) => {
 ...
 */
export async function runQueryToBus(sessionId, params) {
    ...
    try {
        ...
        // 通知观察方窗口该会话的实时流已结束。
        if (clearInflight(sessionId, params.abortController))
