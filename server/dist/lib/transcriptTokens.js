...
const EMPTY_RESULT = { totals: ZERO, lastTurn: null };
/** ~/.claude/projects 目录 */
 ...
 * 找不到返回 null。
 */
function locateTranscript(sessionId) {
    let entries;
    try {
 ...
 */
async function readAssistantUsages(file) {
    ...
    for await (const line of rl) {
        ...
        if (obj.type !== "assistant")
            continue;
        if (obj.isSidechain)
            continue; // 子代理：与 SDK result usage 口径一致
        const id = obj.message?.id;
        const u = obj.message?.usage;
        if (!id || !u)
            continue;
        if (seen.has(id))
            continue; // 同一 message.id 多行只计一次
        seen.add(id);
        out.push({
            messageId: id,
            usage: {
...
export async function getSessionTokenUsage(sessionId) {
    ...
    try {
        ...
        console.warn(`[transcriptTokens] read failed for ${sessionId}:`, err instanceof Error ? err.message : err);
        return EMPTY_RESULT;
    }
    if (rows.length === 0)
        return EMPTY_RESULT;
