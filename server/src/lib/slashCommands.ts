...
/**
 * SDK 的 SlashCommand 类型（只取我们用到的字段）。
 * name 不带前导 `/`，argumentHint 非空字符串，aliases 可选。
 */
 ...
 */
function localizeSdkCommand(nameWithSlash: string, sdkCmd: SdkSlashCommand): SlashCommand {
 ...
 */
export function cacheSlashCommands(cwd: string, sdkCommands: SdkSlashCommand[]): void {
  const commands: SlashCommand[] = [];
  const seen = new Set<string>();
  const push = (cmd: SlashCommand) => {
...
async function resolveFromDisk(cwd: string): Promise<SlashCommand[]> {
  const customNames = new Set<string>();
  const result: SlashCommand[] = [];
  const addIfNew = (cmd: SlashCommand) => {
    if (!customNames.has(cmd.name)) {
  ...
  for (const cmd of await readCustomCommands(cwd)) addIfNew(cmd);
  return result;
}
