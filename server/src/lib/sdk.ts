 ...
 */
const TERMINAL_REASON_LABEL: Record<string, string> = {
  max_turns: "达到最大轮次上限",
...
function buildPermissionRequestHook(sessionIdRef: { current: string | undefined }) {
  return async (input: PermissionRequestHookInput) => {
    ...
    if (decision.updatedInput) allowDecision.updatedInput = decision.updatedInput;
    if (decision.updatedPermissions) allowDecision.updatedPermissions = decision.updatedPermissions;
    return {
      hookEventName: "PermissionRequest" as const,
...
export async function* runQuery(
  ...
  stream.supportedCommands?.()
    .then((cmds) => {
      if (cmds && cmds.length > 0) cacheSlashCommands(params.cwd, cmds);
    })
    .catch((err) => {
      console.warn(
        "[sdk] supportedCommands failed:",
    ...
    switch (msg.type) {
      case "system": {
        if (msg.subtype === "init") {
          ...
          if (m.content) yield { type: "text", text: m.content };
          ...
          const m = msg as { content?: string; level?: string };
          if (m.content) yield { type: "text", text: m.content };
        } else {
          // 兜底：未识别的 system subtype 不再静默丢弃，打印日志便于发现。
          // （AGENTS.md 纪律：后端严禁静默吞错误）
          console.warn(
            "[sdk] unhandled system subtype:",
            msg.subtype,
          );
        }
        break;
      ...
      case "result": {
        ...
        if (msg.subtype !== "success") {
          ...
          const userAborted =
            params.abortController?.signal.aborted === true ||
          ...
          if (!userAborted) {
            ...
            const reason =
              (Array.isArray(e.errors) && e.errors[0]) ||
