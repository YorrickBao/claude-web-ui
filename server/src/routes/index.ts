...
export async function apiRoutes(app: FastifyInstance): Promise<void> {
    ...
    async (req, reply) => {
      ...
      try {
        ...
        if (isInflight) {
          ...
          const memToolIds = new Set<string>();
          let memText = "";
          let memReasoning = "";
          for (const e of memBuf) {
            if (e.type === "tool_use") memToolIds.add(e.id);
            else if (e.type === "text") memText += e.text;
            else if (e.type === "thinking") memReasoning += e.text;
          }
          ...
          if (lastAssistantIdx >= 0) {
            ...
            const hasCurrentTool = parts.some(
              (p) => p.type === "tool-call" && memToolIds.has(p.toolCallId),
            );
            const aText = parts
  ...
  app.get("/api/version", async (_req, reply) => {
    if (!getClaudeVersion()) primeClaudeVersion();
    return reply.send({
      webui: WEBUI_VERSION,
